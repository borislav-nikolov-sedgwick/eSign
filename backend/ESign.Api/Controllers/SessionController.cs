...
public class SessionController : ControllerBase
{
    ...
    public async Task<IActionResult> GetSession(string token)
    {
        ...
        if (session.Status == SessionStatus.Expired || DateTime.UtcNow > session.ExpiresAt)
            return StatusCode(410, new ErrorResponse { Error = "EXPIRED", Message = "Session has expired" });
    ...
    public async Task<IActionResult> DownloadSignedDocument(string token)
    {
        var session = await _sessionService.GetSessionAsync(token);
        if (session == null) return NotFound(new ErrorResponse { Error = "NOT_FOUND", Message = "Session not found" });
        if (session.Status != SessionStatus.Completed) return BadRequest(new ErrorResponse { Error = "NOT_SUBMITTED", Message = "Submit first" });
